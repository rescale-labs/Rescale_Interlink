...
func (w *WorkflowUIComponents) CreateErrorView() fyne.CanvasObject {
	...
	retryBtn := widget.NewButton("Try Again", func() {
		fmt.Printf("DEBUG: 'Try Again' button clicked! PreviousState=%s\n", w.jobsTab.workflow.PreviousState)
...
func (w *WorkflowUIComponents) handleLoadCSVPath() {
	dialog.ShowFileOpen(func(reader fyne.URIReadCloser, err error) {
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					fmt.Printf("PANIC in load CSV goroutine: %v\n", r)
					fyne.Do(func() {
						dialog.ShowError(
			...
			fmt.Printf("DEBUG: Starting CSV load for: %s\n", csvPath)

			// Load and validate CSV
			jobs, err := config.LoadJobsCSV(csvPath)
			fmt.Printf("DEBUG: LoadJobsCSV returned %d jobs, err=%v\n", len(jobs), err)

			// Progress dialog hidden automatically by defer

			if err != nil {
				fmt.Printf("DEBUG: CSV load failed, showing error\n")
				fyne.Do(func() {
			...
			// Validate jobs
			fmt.Printf("DEBUG: About to validate %d jobs\n", len(jobs))
			...
			fmt.Printf("DEBUG: Validation passed, about to update UI\n")
			// Update UI on main thread
			fyne.Do(func() {
				// Set in workflow
				fmt.Printf("DEBUG: About to SetPath, current state=%s\n", w.jobsTab.workflow.CurrentState)
				if err := w.jobsTab.workflow.SetPath(PathLoadCSV); err != nil {
					fmt.Printf("DEBUG: SetPath failed: %v\n", err)
					dialog.ShowError(err, w.jobsTab.window)
					return
				}
				fmt.Printf("DEBUG: SetPath succeeded, now calling SetLoadedCSV\n")

				if err := w.jobsTab.workflow.SetLoadedCSV(csvPath); err != nil {
					fmt.Printf("DEBUG: SetLoadedCSV failed: %v\n", err)
					dialog.ShowError(err, w.jobsTab.window)
					return
				}
				fmt.Printf("DEBUG: SetLoadedCSV succeeded, loading jobs into table\n")

				// Load jobs into table
				w.jobsTab.loadJobsFromSpecs(jobs)
				fmt.Printf("DEBUG: Jobs loaded into table, updating view\n")

				w.jobsTab.updateView()
				fmt.Printf("DEBUG: CSV load complete!\n")
			})
		}()
	}, w.jobsTab.window)
}
...
func (w *WorkflowUIComponents) handleSelectTemplate() {
	dialog.ShowFileOpen(func(reader fyne.URIReadCloser, err error) {
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					fmt.Printf("PANIC in load template goroutine: %v\n", r)
					fyne.Do(func() {
						dialog.ShowError(
...
func (w *WorkflowUIComponents) handleScan(baseDir, pattern string) {
	...
	if w.scanInProgress {
		w.scanMutex.Unlock()
		fmt.Println("DEBUG: Scan already in progress, ignoring duplicate request")
		return
	}
