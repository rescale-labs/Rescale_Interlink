...
// ApplyToCloudFiles filters a slice of cloud files based on the filter configuration.
func ApplyToCloudFiles(files []models.CloudFile, config Config) []models.CloudFile {
	if len(config.Include) == 0 && len(config.Exclude) == 0 && len(config.Search) == 0 {
		// No filters, return all files
		return files
	}

	filtered := make([]models.CloudFile, 0, len(files))
	for _, file := range files {
		if matchesFilter(file.Name, config) {
			filtered = append(filtered, file)
		}
