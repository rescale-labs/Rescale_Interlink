...
// ExtractFileIDFromError attempts to extract a file ID from an API error message.
//
// Some Rescale API errors include the existing file ID in the error message.
// This function tries to parse it out for convenience.
//
// Returns empty string if no file ID found in error.
func ExtractFileIDFromError(err error) string {
	if err == nil {
		return ""
	}

	// TODO: Implement parsing if Rescale includes file ID in error
	// For now, return empty - caller will need to query folder contents
	return ""
}
