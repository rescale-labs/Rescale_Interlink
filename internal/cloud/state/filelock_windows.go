...
// lockGuardFile takes the exclusive lock on an open guard file, waiting for
// whoever holds it. Windows attaches the lock to the file handle, so closing it
// releases the lock — including the close the kernel performs for a process that
// died, which is what stops a taker that crashed mid-reclamation from wedging
// the next one.
