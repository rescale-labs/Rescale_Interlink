...
func (v *CoreTypeValidator) FetchCoreTypes(ctx context.Context) error {
	...
	coreTypes, err := v.client.GetCoreTypes(ctx)
	if err != nil {
...
// GetAvailableCoreTypes returns list of available core types
func (v *CoreTypeValidator) GetAvailableCoreTypes() []string {
	v.mu.RLock()
	defer v.mu.RUnlock()
...
// ValidateBatch validates multiple core types
func (v *CoreTypeValidator) ValidateBatch(coreTypes []string) map[string]error {
	results := make(map[string]error)
	for _, ct := range coreTypes {
		if err := v.Validate(ct); err != nil {
			results[ct] = err
		}
	}
	return results
