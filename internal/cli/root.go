...
// Version information (will be set by main package)
var (
	Version   = "2.4.8"
...
func NewRootCmd() *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Long: `Rescale Interlink v` + Version + ` - Built: ` + BuildTime + `
...
GUI Mode (--gui flag):
  Graphical interface with job monitoring.`,
	...
	rootCmd.PersistentFlags().StringVar(&apiKey, "api-key", "", "Rescale API key (overrides config)")
	...
	rootCmd.PersistentFlags().IntVar(&partsPerFile, "parts-per-file", 0, "Parts per file for concurrent uploads/downloads (0 = auto, range: 1-10)")
...
func CreateResourceManager() *resources.Manager {
	...
	// Validate partsPerFile
	if partsPerFile < 0 || partsPerFile > 10 {
		fmt.Fprintf(os.Stderr, "Warning: --parts-per-file must be between 0 and 10, using auto-detect\n")
		partsPerFile = 0
	}
...
// GetMaxThreads returns the max-threads flag value
func GetMaxThreads() int {
	return maxThreads
}

// GetNoAutoScale returns the no-auto-scale flag value
func GetNoAutoScale() bool {
	return noAutoScale
}

// GetPartsPerFile returns the parts-per-file flag value
func GetPartsPerFile() int {
	return partsPerFile
}

// HandleError handles CLI errors in a consistent way.
