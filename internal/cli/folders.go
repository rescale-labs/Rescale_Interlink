...
Examples:
  # Upload directory to root (My Library)
  ...
  rescale-int folders upload-dir ./data --continue-on-error --include-hidden`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if exists {
				...
				if !skipExisting {
					...
					if action == ConflictAbort {
						return fmt.Errorf("upload cancelled by user")
					}
					if action == ConflictSkipOnce || action == ConflictSkipAll {
			...
			if sequential {
				...
				uploadResult, created, err := uploadDirectoryPipelined(
					...
					files, folderConcurrency, maxConcurrent, continueOnError, skipExisting, cfg, logger)
				if err != nil {
					return err
				}
				result = uploadResult
	...
	cmd.Flags().BoolVar(&skipExisting, "skip-existing", false, "Automatically skip existing folders/files without prompting")
...
Examples:
  ...
  rescale-int folders download-dir abc123 --continue-on-error --overwrite`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if conflictFlags > 1 {
				return fmt.Errorf("only one of --overwrite or --skip can be specified")
			}
			...
			result, err := downloadFolderRecursive(
				ctx, folderID, outputDir, overwriteAll, skipAll, continueOnError, maxConcurrent, skipChecksum, apiClient, logger)
			if err != nil {
				return err
			}
