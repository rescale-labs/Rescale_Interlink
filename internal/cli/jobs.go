...
Example:
  ...
  rescale-int jobs list --limit 10`,
	...
	cmd.Flags().IntVar(&limit, "limit", 0, "Limit number of jobs displayed (0 = all)")
...
Example:
  # Delete single job
  rescale-int jobs delete --id XxYyZz

  # Delete multiple jobs
  rescale-int jobs delete --id XxYyZz --id AaBbCc --id DdEeFf`,
		RunE: func(cmd *cobra.Command, args []string) error {
			logger := GetLogger()
	...
	cmd.Flags().StringArrayVar(&jobIDs, "id", []string{}, "Job ID to delete (can be specified multiple times, required)")
	cmd.Flags().StringArrayVar(&jobIDs, "job-id", []string{}, "Job ID to delete (alias for --id)")
...
Examples:
  ...
  rescale-int jobs submit --script run.sh --files *.dat --end-to-end --download`,
	...
	cmd.Flags().StringVar(&jobID, "job-id", "", "Existing job ID to submit (use with --submit only)")
	...
	cmd.Flags().BoolVar(&endToEnd, "end-to-end", false, "Full workflow: upload → create → submit → monitor → download")
	cmd.Flags().BoolVarP(&endToEnd, "E", "E", false, "Alias for --end-to-end (deprecated, use --end-to-end)")
...
Example:
  ...
  rescale-int jobs tail --job-id XxYyZz --interval 5`,
	...
	cmd.Flags().StringVar(&jobID, "job-id", "", "Job ID (required)")
	cmd.Flags().IntVar(&pollInterval, "interval", 10, "Polling interval in seconds")
	cmd.MarkFlagRequired("job-id")
...
Examples:
  ...
  rescale-int jobs download --id XxYyZz --outdir ./results

  # Download with high concurrency
  rescale-int jobs download --id XxYyZz --max-concurrent 10

  # Download only specific file types
  rescale-int jobs download --id XxYyZz --filter "*.dat,*.log"

  # Download all except debug files
  rescale-int jobs download --id XxYyZz --exclude "debug*,temp*"

  # Download files containing "results" in filename
  rescale-int jobs download --id XxYyZz --search "results"

  # Combined filters (include .dat files, exclude debug files, must contain "final")
  rescale-int jobs download --id XxYyZz --filter "*.dat" --exclude "debug*" --search "final"

  # Download all files, overwriting existing
  rescale-int jobs download --id XxYyZz --overwrite

  # Download specific file by ID
  rescale-int jobs download --id XxYyZz --file-id AbCdEf --output /path/to/file.dat`,
		RunE: func(cmd *cobra.Command, args []string) error {
			logger := GetLogger()

			if jobID == "" {
				return fmt.Errorf("--id or --job-id is required")
			}
			...
			err = download.DownloadFileWithProgressAndTransfer(ctx, fileID, outputPath, apiClient, func(fraction float64) {
	...
	cmd.Flags().StringVar(&jobID, "job-id", "", "Job ID (alias for --id)")
...
func runCreateOnlyWorkflow(
	...
	if len(inputFiles) > 0 {
		...
		fileIDs, err := UploadFilesWithIDs(ctx, inputFiles, "", maxConcurrent, apiClient, logger, false)
		if err != nil {
