...
// Invalidate removes a folder from the cache (used after modifications)
...
func uploadDirectoryPipelined(
	...
	go func() {
		...
		for event := range folderReadyChan {
			...
			relativePath, _ := filepath.Rel(rootPath, event.LocalPath)
			if relativePath == "." {
			...
			for i, filePath := range filesInFolder {
				...
				go func(idx int, fpath string, remoteFolderID string) {
					...
					_, _ = filepath.Rel(rootPath, fpath) // For potential future use
					...
					if exists {
						...
						if action == FileIgnoreOnce || action == FileOverwriteOnce {
							action, _ = promptFileConflict(fileName, relativePath)
					...
					// Upload file
					cloudFile, uploadErr := upload.UploadFileToFolder(ctx, fpath, remoteFolderID, apiClient,
...
func uploadFiles(
	...
	for i, filePath := range files {
		...
		go func(idx int, fpath string) {
			...
			fileName := filepath.Base(fpath)
			relativePath, _ := filepath.Rel(rootPath, fpath)
			...
			cloudFile, err := upload.UploadFileToFolder(ctx, fpath, remoteFolderID, apiClient, func(prog float64) {
			...
			if err != nil {
				...
				if !cfg.CheckConflictsBeforeUpload && api.IsFileExistsError(err) {
					...
					case FileOverwriteOnce, FileOverwriteAll:
						...
						cloudFile, retryErr := upload.UploadFileToFolder(ctx, fpath, remoteFolderID, apiClient, func(prog float64) {
							fileBar.UpdateProgress(prog)
						}, uploadUI.Writer())
