...
// NOTE: The current pkcs7Unpad implementation does not validate that all padding bytes
// have the correct value (only checks padding length). This could be improved for
// better security (protection against padding oracle attacks), but that would be a
// separate code enhancement, not a test fix.
